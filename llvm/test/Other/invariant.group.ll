...
; CHECK-LABEL: define i8 @optimizableForStrip()
define i8 @optimizableForStrip() {
    %ptr = alloca i8
    store i8 42, i8* %ptr, !invariant.group !0
; CHECK: call i8* @llvm.strip.invariant.group
    %ptr2 = call i8* @llvm.strip.invariant.group.p0i8(i8* %ptr)
    store i8 43, i8* %ptr
; CHECK-NOT: call i8* @llvm.strip.invariant.group
    %ptr3 = call i8* @llvm.strip.invariant.group.p0i8(i8* %ptr)
; CHECK: call void @clobber(i8* {{.*}}%ptr)
    call void @clobber(i8* %ptr)
; CHECK: call void @use(i8* {{.*}}%ptr2)
    call void @use(i8* %ptr2)
...
; This test check if optimizer is not proving equality based on mustalias
; CHECK-LABEL: define void @dontProveEqualityForStrip(
define void @dontProveEqualityForStrip(i8* %a) {
  %b = call i8* @llvm.strip.invariant.group.p0i8(i8* %a)
  %r = icmp eq i8* %b, %a
;CHECK: call void @useBool(i1 %r)
  call void @useBool(i1 %r)
  ret void
